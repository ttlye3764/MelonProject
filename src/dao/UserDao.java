...
public class UserDao {
	...
	public UserVO selectUser(HashMap<String, String> param) {
		...
		for (int i = 0; i < database.tb_user.size(); i++) {
			...
			for (String key : param.keySet()) {
				...
				if (key.equals("ID")) {
					if (!user.getU_id().equals(value))
						flag = false;
				} else if (key.equals("PASSWORD")) {
					if (!user.getU_pw().equals(value))
						flag = false;
				} else if (key.equals("NAME")) {
					if (!user.getU_name().equals(value))
						flag = false;
				} else if (key.equals("UNAME")) {
					if (!user.getU_n_name().equals(value))
						flag = false;
				}
			}
			if (flag)
	...
	public void deleteUser(UserVO param) { // 회원 탈퇴 03.09
		...
		if (Session.LoginUser.getU_id().equals(param.getU_id())
				&& Session.LoginUser.getU_pw().equals(param.getU_pw())) {
			for (int i = 0; i < database.tb_user.size(); i++) {
				user = database.tb_user.get(i);
				if (user.getU_id().equals(Session.LoginUser.getU_id())) {
