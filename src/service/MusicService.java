...
public class MusicService {
	...
	public void searchMusic() { // 음악 찾기 및 플레이 리스트에 추가하는부분
		...
		if (music == null) {
			...
			System.out.println(music.getM_number() + "\t" + music.getM_name()
					+ "\t" + music.getM_singer() + "\t" + music.getM_lyrics()
					+ "\t" + music.getM_composition() + "\t"
					+ music.getM_genre() + "\t" + music.getM_date() + "\t"
					+ music.getM_count());
			...
			if (Session.LoginUser != null ) {
				...
				System.out.println("로그인을 하시면 듣기와 담기 기능을 이용하실 수 있습니다.");
			}
		}
	}
	...
	public void Show_R_musicList() { // 최근들은 노래 리스트 보기
		...
		ArrayList<MusicVO> musicList = musicDao.MusicList();
		ArrayList<R_playListVO> r_playlist = musicDao.R_PlayList(); // m_number
																	// 노래등록번호
	...
	public void Genre_MusicChart() { // 장르별 차트 보기
		...
		for (int i = 0; i < bal.size(); i++) {
			...
			System.out.println((i + 1) + "위" + "\t" + music.getM_name() + "\t"
					+ music.getM_singer());
		}
		System.out
				.println("-----------------------댄스 순위 차트-----------------------");
	...
	public void MusicChart() { // 인기 차트 보기
		...
		for (int i = 0; i < musicChart.length; i++) {
			...
			System.out.println((i + 1) + "위" + "\t" + music.getM_name() + "\t"
					+ music.getM_singer() + "\t" + music.getM_lyrics() + "\t"
					+ music.getM_composition() + "\t" + music.getM_genre()
					+ "\t" + music.getM_date() + "\t" + music.getM_count());
		}
	}
	...
	public void R_MusicChart() { // 최신곡 차트 보기
		...
		for (int i = 0; i < musicChart.length; i++) {
			MusicVO music = musicChart[i];
			System.out.println((i + 1) + "\t" + music.getM_name() + "\t"
					+ music.getM_singer() + "\t" + music.getM_lyrics() + "\t"
					+ music.getM_composition() + "\t" + music.getM_genre()
					+ "\t" + music.getM_date() + "\t" + music.getM_count());
		}
